...
import perfomance.test.queue.waiting.dto.request.QueueActivityRequest;
...
public class WaitingQueueController {
    ...
    public Mono<ResponseEntity<WaitingQueueStatusResponse>> recordActivity(
            @PathVariable String eventId,
            @ModelAttribute QueueActivityRequest request
    ) {
        return service.recordActivity(eventId, request.userId(), request.sequence(), request.authority())
                .map(WaitingQueueStatusResponse::from)
                .map(ResponseEntity::ok);
    }
}
