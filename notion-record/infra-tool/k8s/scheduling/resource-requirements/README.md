...
spec:
  containers:
  - name: main-app                # 메인 애플리케이션 컨테이너
    image: nginx
    resources:
      requests:
        cpu: "1"                  # 최소 1 CPU 보장
        memory: "512Mi"           # 최소 512Mi 메모리 보장
      limits:
        cpu: "2"                  # 최대 2 CPU로 제한 (초과 시 스로틀링)
        memory: "1Gi"             # 최대 1Gi 메모리로 제한 (초과 시 OOMKill)

  - name: log-sidecar             # 로그 수집용 보조 컨테이너
    image: busybox
    resources:
      requests:
        cpu: "100m"               # 최소 0.1 CPU 보장
        memory: "64Mi"            # 최소 64Mi 메모리 보장
      limits:
        cpu: "500m"               # 최대 0.5 CPU로 제한
        memory: "128Mi"           # 최대 128Mi 메모리로 제한
```
...
- **목적:** 정의서에 자원 설정이 누락된 경우 자동으로 기본값을 설정하거나 최소/최대 범위를 강제함
- **주요 필드:**
