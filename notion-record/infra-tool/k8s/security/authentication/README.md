...
## Kubernetes 클러스터 인증(Authentication)

- Kubernetes 클러스터는 여러 노드(물리/가상)와 여러 컴포넌트가 함께 동작하는 구성
- 클러스터에 접근하는 사용자 유형
    - 관리자(administrators)가 관리 작업 수행
    - 개발자(developers)가 테스트 또는 애플리케이션 배포
    - 애플리케이션을 사용하는 end users 존재
    - 통합 목적의 third party 애플리케이션 존재
